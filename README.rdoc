...
b) make it parent, and make 2 subtrees(values less then middle value - left tree. greater then middle value - right tree)
...
1. insert parent's value to actual node
2. move smallest/biggest(for right/left) value from brother to parent
b) If left and right brother(node) has not number of elements grater then minimum
1. move other values from node and parent's value to brother node
2. becouse we move value from parent node it could have less then minimum keys, so we must merge parent node with its brother
